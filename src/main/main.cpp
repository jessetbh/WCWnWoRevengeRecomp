...
#endif
    window = SDL_CreateWindow("WCW/nWo Revenge: Recompiled",
...
static void wcw_setup_logging(bool show_console) {
    ...
    std::filesystem::path log_path = app_folder / "RevengeRecompiled.log";
...
int main(int argc, char** argv) {
    ...
    game.rom_hash = 0x55F2CF93CC859D57ULL;          // XXH3_64bits of the NTSC-U ROM
    game.internal_name = "WCW / nWo  REVENGE";
    game.display_name = "WCW/nWo Revenge";
    ...
    std::u8string game_id = u8"wcw.nwo.revenge.us";
    ...
    if (autoboot != nullptr) {
        ...
        recompui::register_launcher_init_callback([](recompui::LauncherMenu* menu) {
            // Game logo in place of the default program-name text title (WT's exact
            // pattern, commit 296d9c0 there). The image must be queued before the
            // Image element referencing its src is created.
            menu->remove_default_title();
            static const std::string logo_src = "?/game/wcw.nwo.revenge.us/logo";
            ...
            logo->set_height(520.0f * 489.0f / 1200.0f);  // icons/logo.png aspect
